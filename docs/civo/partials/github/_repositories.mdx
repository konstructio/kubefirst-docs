...
These {props.name} repositories are being managed in Terraform. As you need additional GitHub repositories, just add a new section of Terraform code to `civo-github/terraform/{props.name.toLowerCase()}/repos.tf`:

{
  if props.name == "github" {
    return <TerraformGitLab />
  } else if props.name == "gitlab" {
    return <TerraformGitHub />
  }
}
