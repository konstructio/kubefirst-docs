...
The folder `civo-github/terraform/users/admins` contains two separate files that represent admin users: `admin-one.tf` (commented-out), and the kubefirst_bot user in the `kbot.tf` file. Here's the module from `admin-one.tf`:
